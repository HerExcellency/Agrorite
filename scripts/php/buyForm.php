...
$field_companyName = $_POST['companyName'];
$field_country = $_POST['country'];
$field_phone = $_POST['phone'];
$field_proposalAvail = $_POST['proposalAvail'];
...
/**
 * VALIDATION FOR COMPANY NAME STARTS HERE
 */
if (empty($field_companyName)) {
    return helper::Output_Error(null, "Company name is required");
}
if($field_companyName === ''){
    return helper::Output_Error(null, "Company name field cannot be empty");
}
if(strlen($field_name) < 2){
    return helper::Output_Error(null, "Please put in a valid company name");
}
...
if (empty($field_country)) {
    return helper::Output_Error(null, "Location is required");
}
if($field_country === ''){
    return helper::Output_Error(null, "Country field cannot be empty");
}
...
if (strlen($field_phone) < 6 || strlen($field_phone) > 16) {
...
// Validation for message starts here
...
$mail_to = 'ijeomaonuajurx@gmail.com';
$subject = 'Message from A Buyer';

$body_message = 'From: '.$field_name."\n";
$body_message .= 'E-mail: '.$field_email."\n";
$body_message .= 'Company Name: '.$field_companyName."\n";
$body_message .= 'Location: '.$field_country."\n";
$body_message .= 'Mobile: '.$field_phone."\n";
$body_message .= 'Proposal: '.$field_proposalAvail."\n";
