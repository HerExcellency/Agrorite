...
if(count($fetch) === 0){
    $androidResponse["message"] = 'Invalid user';
        // echo JSON response
...
if(count($fetchFarm) === 0){
    $androidResponse["message"] = 'Invalid farm selected';
        // echo JSON response
        echo json_encode($androidResponse);
}
...
if($submitFarmDetails === TRUE){
    ...
    $text = '
			...
			try {
				...
				$mail->send();
				$androidResponse["message"] = 'Please check your email for your invoice';
        // echo JSON response
        echo json_encode($androidResponse);
			} catch (Exception $e) {
